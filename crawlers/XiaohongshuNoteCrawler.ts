...
export class XiaohongshuNoteCrawler {
  ...
  async crawl(opts?: { onProgress?: (msg: string) => void }): Promise<XiaohongshuNote> {
    ...
    // 图片
    const imageUrlList = extractNoteImages(noteId)
    let coverUrl = imageUrlList[0] || null
    const imageUrls = imageUrlList.length > 0 ? imageUrlList.join(',') : null

    // 视频地址只读取与当前笔记匹配的 Open Graph 元数据。
    let videoUrl: string | null = null

    const masks = Array.from(document.querySelectorAll('.note-detail-mask[note-id]'))
    const activeMask = masks.find(el => el.getAttribute('note-id') === noteId)
    const mediaContainer = activeMask?.querySelector('#noteContainer') ||
      (masks.length === 0 ? document.querySelector('#noteContainer.note-container') : null)
    const player = mediaContainer?.querySelector('.media-container [elementtiming="note-cover"]')
    const videoEl = player?.querySelector<HTMLVideoElement>('video')
    ...
    try {
      ...
      const ogNoteId = pageUrl.pathname.match(/^\/explore\/([\w-]+)\/?$/)?.[1]
      if (['http:', 'https:'].includes(pageUrl.protocol) &&
          /(^|\.)xiaohongshu\.com$/.test(pageUrl.hostname) && ogNoteId === noteId) {
        const candidate = document.querySelector<HTMLMetaElement>('meta[property="og:video"]')?.content
        if (candidate && /^https?:\/\//i.test(candidate)) videoUrl = httpUrl(candidate)
      }
    } catch { /* Missing or mismatched metadata must not fall back to another video source. */ }
    const poster = player?.querySelector<HTMLElement>('xg-poster')
    const background = poster ? getComputedStyle(poster).backgroundImage : ''
    const posterUrl = background.match(/url\(\s*["']?(.*?)["']?\s*\)/)?.[1]
    coverUrl = httpUrl(posterUrl) || httpUrl(videoEl?.getAttribute('poster')) || coverUrl
    const declaredType = mediaContainer?.getAttribute('data-type')
    const mediaType: 'image' | 'video' | undefined =
      declaredType === 'video' ? 'video' :
      declaredType === 'normal' ? 'image' :
      videoEl || videoUrl || noteDetail?.type === 'video' ? 'video' :
      imageUrlList.length > 0 || noteDetail?.type === 'normal' ? 'image' : undefined
    const workType = mediaType === 'video' ? '视频' : mediaType === 'image' ? '图文' : undefined
      ...
      if (hovercard) {
        ...
        if (style.display !== 'none' && style.opacity !== '0') {
          for (let j = 0; j < 10; j++) {
            ...
            if (data.followers || data.following || data.likes) return { followers: data.followers ?? undefined, following: data.following ?? undefined, likes: data.likes ?? undefined }
          }
          return null
        }
      }
