...
// 移除无用标签
turndownService.remove(node => ['SCRIPT', 'STYLE', 'SVG', 'IFRAME'].includes(node.nodeName.toUpperCase()))
...
export class WechatArticleCrawler {
  ...
  async crawl(_opts?: { onProgress?: (msg: string) => void }): Promise<WechatArticle> {
    ...
    if (!contentEl || (!contentEl.textContent?.trim() && !contentEl.querySelector('img, video, iframe')))
      throw new Error('未识别到文章内容，请等待页面加载完成后重试。')
