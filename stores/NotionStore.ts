import { notionRequest, resolveTargets } from '@/services/notionDiscovery'
...
// 笔记详情页 Schema（完整数据）
const NOTE_SCHEMA = {
  '作品类型': { type: 'select', notionType: { select: { options: [{ name: '图文' }, { name: '视频' }] } } },
  ...
  '视频': { type: 'files', notionType: { files: {} } },
...
async function getFieldMap(dbId: string, type: string): Promise<Record<string, string>> {
  ...
  return (result[key] || {}) as Record<string, string>
}
...
async function uploadImagesByUrl(
  ...
  if (opts?.progressLabel) {
    showPanelTip(`正在处理${opts.progressLabel} 0/${urls.length}（下载并上传）...`, opts.fromTabId)
  }
  return runWithConcurrency(urls, opts?.concurrency ?? DEFAULT_IMAGE_UPLOAD_CONCURRENCY, async (url, index) => {
    ...
    for (let attempt = 0; attempt <= retry; attempt++) {
      try {
        ...
        completed++
        if (opts?.progressLabel) {
          showPanelTip(`正在处理${opts.progressLabel} ${completed}/${urls.length}（下载并上传）...`, opts.fromTabId)
        }
    ...
    api.warnings.push('部分图片上传失败，已保留原始链接。')
...
async function uploadVideoByUrl(
  ...
  try {
    ...
    clearTimeout(timer)
    api.warnings.push('视频上传失败，已保留原始链接。')
...
class NotionAPI {
  ...
  async getDatabase() {
    ...
    this.schemaCache = await this.request('GET', `/data_sources/${this.databaseId}`)
    return this.schemaCache
  }

  async updateDatabaseSchema(properties: any) {
    try {
      const result = await this.request('PATCH', `/data_sources/${this.databaseId}`, { properties })
  ...
  async queryDatabase(filter: any) {
    return this.request('POST', `/data_sources/${this.databaseId}/query`, { filter })
  }

  async createPage(properties: any, children: any[] | null = null, markdown: string | null = null) {
    const payload: any = { parent: { type: 'data_source_id', data_source_id: this.databaseId }, properties }
    ...
    if (saveFieldMapCallback) {
      ...
      if (titleFields.length > 0) {
        ...
        if (titleProp) {
          ...
          // Preserve the user's title column name; map it rather than rename it.
      ...
      for (const [name, config] of Object.entries(schema) as any) {
        ...
        if (existing[name]) {
          ...
          if (expectedType && actualType && expectedType !== actualType) {
            throw new Error(`字段「${name}」类型不兼容：需要 ${expectedType}，当前为 ${actualType}。请选择其他数据库或调整字段。`)
          }
...
async function saveXhsNoteToNotion(
  ...
  for (let retry = 0; retry < 2; retry++) {
    try {
      ...
      showPanelTip('正在准备笔记字段与媒体...', fromTabId)
...
async function saveXhsAccountToNotion(
  ...
  for (let retry = 0; retry < 2; retry++) {
    try {
      ...
      showPanelTip('正在准备账号字段与媒体...', fromTabId)
      const props = await toNotionProperties(
        data,
        schema,
        api.fieldNameMap,
...
export const notionStore: StoreAdapter = {
  async save(
    ...
    let databaseId = parseNotionDatabaseId(store?.dataSourceId)
    if (!databaseId && token && store?.databaseId) {
      const choices = await resolveTargets(token, store.connectionId || 'legacy', store.databaseId)
      if (choices.length !== 1) return { ok: false, error: '该数据库包含多个数据源，请重新选择具体保存位置。' }
      databaseId = parseNotionDatabaseId(choices[0].dataSourceId)
    }

    if (!token || !databaseId) {
      return { ok: false, error: '请连接 Notion 并选择保存位置' }
    }
    ...
    try {
      ...
      const warning = [...new Set(api.warnings)].join('；')
      return Array.isArray(result) ? result.map(r => ({ ...r, warning: warning || undefined })) : { ...result, warning: warning || undefined }
    } catch (e: any) {
...
/** Read-only validation, shared by selection and saving. Never creates or renames properties. */
export async function inspectNotionTarget(token: string, dataSourceId: string, type: string) {
  const source = await notionRequest(token, `/data_sources/${dataSourceId}`)
  if (source.in_trash || source.archived) throw new Error('这个数据源已被移入废纸篓。')
  const schema = getSchemaByType(type)
  if (!schema) throw new Error('不支持的采集类型')
  const conflicts: string[] = []; const missing: string[] = []
  for (const [name, field] of Object.entries(schema) as any) {
    if (field.type === 'title') continue
    const actual = source.properties?.[name]
    const expected = Object.keys(field.notionType || {})[0]
    if (!actual) missing.push(name)
    else if (actual.type !== expected) conflicts.push(`「${name}」需要 ${expected}，当前为 ${actual.type}`)
  }
